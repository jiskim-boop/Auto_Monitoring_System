...
def fetch_quote(sym):
    ...
    try:
        ...
        prev=res["meta"].get("chartPreviousClose") or (closes[-2] if len(closes)>1 else price)
...
def summarize_filing(link):
    ...
    if APIKEY and txt:
        s=claude("다음은 미국 BDC의 8-K 공시 본문이다. 무슨 사건인지 한국어 한 줄(45자 내외)로 핵심만 요약하라. "
                 "실적·배당·인수·자금조달·소송·임원변경 등 위주.\n\n"+txt[:3500], max_tokens=120)
        if s: return s
...
def summarize_news(news):
    ...
    if APIKEY and (hc or hf):
        s=claude("다음은 AI 인프라 신용·사모대출 환매(CREDIT)와 하이퍼스케일러 capex(FUNDAMENTAL) 관련 최신 영문 "
            "헤드라인이다. 현재 상황을 한국어 한 문장(80자 내외)으로 과장 없이 사실 위주 요약. 신용 스트레스 정도와 "
            "capex 흐름을 함께.\n\n[CREDIT]\n"+"\n".join("- "+h for h in hc)+"\n\n[FUNDAMENTAL]\n"+"\n".join("- "+h for h in hf))
        if s: return {"text":s,"by":"claude"}
